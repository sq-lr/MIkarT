...
| `objects[].type` | string | The snake_case label an object was given, either by the vision model (an object it found in the photo, e.g. `palm_tree`, `lantern`), the text-asset extractor (a prop the player's description named that may not be in the photo, e.g. `whale_statue`), or the filler-asset suggester (a generic prop found in a public library, e.g. `traffic_cone`, see `docs/decisions/0008-generic-filler-from-poly-pizza.md`) — indistinguishable in this field. Open vocabulary. `AssetResolver` has tuned placeholders for common labels and a keyword heuristic for the rest. |
